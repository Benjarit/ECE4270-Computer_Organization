...
void WB() //MEM_WB
{
	if(MEM_WB.wb == 1){
		...
		if(branch){ // if branching instruction in EX stage in the last cycle, set ID control flag back to 1
			IF_ID.id = 1;
		}
		if(ENABLE_FORWARDING){
			hold = MEM_WB.ALUOutput;
		}
	}
	INSTRUCTION_COUNT++;
}
...
uint32_t read_cache(uint32_t add){
	...
	if((L1Cache.blocks[index].valid != 1) || (L1Cache.blocks[index].tag != tag)){			
		...
		L1Cache.blocks[index].words[1] = mem_read_32(add + 0x4);
	...
	if(EX_MEM.m == 1){
		...
		switch(EX_MEM.opcode){
			case 0x00000020:
				...
				if(EX_MEM.left){
					printf("# LB\n");
					word = read_cache(EX_MEM.ALUOutput);
					EX_MEM.LMD = (word & 0x00000080) == 0x80 ? 0xFFFFFF00 | word : word;
				}
				break;	
			case 0x00000021:
				if(EX_MEM.left){
					printf("# LH\n");
					word = read_cache(EX_MEM.ALUOutput);
					EX_MEM.LMD = (word & 0x00008000) == 0x8000 ? 0xFFFF0000 | word : word;	
				}
				break;
			case 0x00000023:
				if(EX_MEM.left){
					printf("# LW\n");
					EX_MEM.LMD = read_cache(EX_MEM.ALUOutput);
		...
		switch(ID_EX.opcode){
			...
			case 0x00000009:
				...
				if(ID_EX.right){
					...
					CURRENT_STATE.REGS[ID_EX.RD] = CURRENT_STATE.PC-4; // already at PC+8 ###### <- this works with forwarding off but not on ???????????????????????????????????????
					jmpBy =  ID_EX.A - (CURRENT_STATE.PC);
				// ADDIU
				}else{
					printf("# ADDIU\n");
			...
			case 0x0000001B: 
				...
				if((ID_EX.prevIns == 0x00000010) | (ID_EX.prevIns == 00000012)){
					...
					if(ID_EX.B == 0){
						...
						ID_EX.type = 100;
						CURRENT_STATE.HI = ID_EX.A / ID_EX.B; // Div = A/B
						CURRENT_STATE.LO = ID_EX.A % ID_EX.B; // REM = A%B
					}
				}
				break;
			...
			case 0x00000010:
				...
				//CURRENT_STATE.REGS[ID_EX.RD] = CURRENT_STATE.HI;
				ID_EX.type = 0;
			...
			case 0x00000012:
				// MFLO
				printf("# MFLO\n");
				//CURRENT_STATE.REGS[ID_EX.RD] = CURRENT_STATE.LO;				
...
void IF() // IF_ID
{
	...
	if(IF_ID.i == 1){ // set to a 1 in main()
		if(IF_ID.id == 0){ // if branch, read from new location
			IF_ID.id = 1;
